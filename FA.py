...
class FA():
    ...
    # 选择测试函数
        ...
        # 函数 Ackley's Function
            ...
            obj_val = -20*np.exp(-0.2*np.sqrt(sum(X**2)/dim)) - np.exp(np.sum(np.cos(2*np.pi*X))/dim) + 20 + np.exp(1)
        ...
        # 函数 Penalized 1 Function
            ...
            obj_val = (np.pi/dim)*(10*((np.sin(np.pi*(1+(X[0]+1)/4)))*2)+sum((((X[:-1]+1)/4)**2)*(1+10*((np.sin(np.pi*(1+(X[1:]+1)/4))))**2))+((X[-1]+1)/4)**2)+sum(Ufun(X, 10, 100, 4))
            # y1 = y(X[0])
            # yD = y(X[-1])
            # yi = y(X[:-1])
            # yi_1 = y(X[1:])
        ...
        # 函数 Penalized 2 Function
            ...
            obj_val = 0.1*((np.sin(3*np.pi*X[0]))**2 + sum((X[:-1]-1)**2*(1+(np.sin(3*np.pi*X[1:]))**2))+((X[dim-1]+1)**2)*(1+(np.sin(2*np.pi*X[-1]))**2))+sum(Ufun(X, 5, 100, 4))
            # X1 = X[0]
            # XD = X[-1]
            # Xi = X[:-1]
            # Xi_1 = X[1:]
...
if __name__ == '__main__':
    ...
    for i in list_test:
        Run = FA(10, 10, 2.0, 2.0, func_sel=i)
        Run.fa_main()
